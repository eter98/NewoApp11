...
public class CuentaAsociadaResourceIT {
     ...
     *
     * @see io.github.jhipster.application.repository.search.CuentaAsociadaSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void getAllCuentaAsociadasByFechaVencimientoIsGreaterThanOrEqualToSomething() throws Exception {
        ...
        // Get all the cuentaAsociadaList where fechaVencimiento greater than or equals to DEFAULT_FECHA_VENCIMIENTO
        defaultCuentaAsociadaShouldBeFound("fechaVencimiento.greaterOrEqualThan=" + DEFAULT_FECHA_VENCIMIENTO);

        // Get all the cuentaAsociadaList where fechaVencimiento greater than or equals to UPDATED_FECHA_VENCIMIENTO
        defaultCuentaAsociadaShouldNotBeFound("fechaVencimiento.greaterOrEqualThan=" + UPDATED_FECHA_VENCIMIENTO);
    }
    ...
    public void getAllCuentaAsociadasByFechaVencimientoIsLessThanSomething() throws Exception {
        ...
        // Get all the cuentaAsociadaList where fechaVencimiento less than or equals to DEFAULT_FECHA_VENCIMIENTO
        defaultCuentaAsociadaShouldNotBeFound("fechaVencimiento.lessThan=" + DEFAULT_FECHA_VENCIMIENTO);

        // Get all the cuentaAsociadaList where fechaVencimiento less than or equals to UPDATED_FECHA_VENCIMIENTO
    ...
    @Test
    @Transactional
    public void getAllCuentaAsociadasByFacturacionIsEqualToSomething() throws Exception {
        // Initialize the database
        Facturacion facturacion = FacturacionResourceIT.createEntity(em);
        em.persist(facturacion);
        em.flush();
        cuentaAsociada.addFacturacion(facturacion);
        cuentaAsociadaRepository.saveAndFlush(cuentaAsociada);
        Long facturacionId = facturacion.getId();

        // Get all the cuentaAsociadaList where facturacion equals to facturacionId
        defaultCuentaAsociadaShouldBeFound("facturacionId.equals=" + facturacionId);

        // Get all the cuentaAsociadaList where facturacion equals to facturacionId + 1
        defaultCuentaAsociadaShouldNotBeFound("facturacionId.equals=" + (facturacionId + 1));
    }
