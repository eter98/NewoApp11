...
public class ChatGrupoResourceIT {
    ...
    public void createChatGrupo() throws Exception {
        ...
        restChatGrupoMockMvc.perform(post("/api/chat-grupos")
            ...
            .content(TestUtil.convertObjectToJsonBytes(chatGrupo)))
    ...
    @Test
    @Transactional
    public void getAllChatGruposByChatListGrupoIsEqualToSomething() throws Exception {
        // Initialize the database
        ChatListGrupo chatListGrupo = ChatListGrupoResourceIT.createEntity(em);
        em.persist(chatListGrupo);
        em.flush();
        chatGrupo.setChatListGrupo(chatListGrupo);
        chatGrupoRepository.saveAndFlush(chatGrupo);
        Long chatListGrupoId = chatListGrupo.getId();

        // Get all the chatGrupoList where chatListGrupo equals to chatListGrupoId
        defaultChatGrupoShouldBeFound("chatListGrupoId.equals=" + chatListGrupoId);

        // Get all the chatGrupoList where chatListGrupo equals to chatListGrupoId + 1
        defaultChatGrupoShouldNotBeFound("chatListGrupoId.equals=" + (chatListGrupoId + 1));
    }
    ...
    public void updateChatGrupo() throws Exception {
        // Initialize the database
        chatGrupoService.save(chatGrupo);
        // As the test used the service layer, reset the Elasticsearch mock repository
        reset(mockChatGrupoSearchRepository);
        ...
        restChatGrupoMockMvc.perform(put("/api/chat-grupos")
            .contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(updatedChatGrupo)))
            .andExpect(status().isOk());
